...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    let essentialInfoEl = Array.from(
      document.querySelectorAll(".cart__non-essential-info")
    );
  ...
  const handleChange = (id, amount) => {
    ...
    let cleanAmount = Number(
      amount
        .split("")
        .filter((el) => el.search(onlyNumbersRegex) === 0)
        .join("")
    );
                ...
                {productsInCart.map((el, index) => {
                  ...
                  return (
                    <tr key={product.id}>
                      ...
                      <td>
                        <Link
                          to={`/singleProduct/${product.fields.productCode}`}
                        >
                          {product.fields.name}
                        </Link>
                      </td>
                      <td>
                        <div className="cart__quantity-container">
                          <input
                            ...
                            value={getVal(product.id) || 0}
                          />
                          <button
                            className="cart__update-quantity-amt"
                            onClick={() => clickHandle(product.id)}
                          >
                            <BsArrowRepeat />
                          </button>
                          <button
                            className="cart__update-remove-item"
                      ...
                      <td className="cart__non-essential-info">
                        <p className="cart__order-description">
                          {product.fields.description.slice(0, descCharMax)}...
                        </p>
                      </td>
                      <td className="cart__non-essential-info">
...
const Wrapper = styled.section`
  ...
  .cart__summary-container table,
  .cart__summary-container th,
  .cart__summary-container td {
    border: 1px solid #ccc;
    border-collapse: collapse;
